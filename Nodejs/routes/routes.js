...
router.get('/:key', function(req, res) {
	if(req.params.key == key) {
		res.render('map', { title: 'Track Pack - Long Range Wi-Fi', user: user_coords, dish: dish_coords, key: req.params.key});
	} else {
...
router.get('/simple/:key', function(req, res) {
	if(req.params.key == key) {
		res.render('simple', { title: 'Track Pack - Long Range Wi-Fi', user: user_coords, dish: dish_coords, key: req.params.key});
	} else {
		res.status(404).render('error', {message: 'Access not Authorized', error: {status:"Incorrect Key Specified",stack:"Please Specify correct key."}});
  }
});
